...
export default function DisplayPage() {
  ...
  const totalAvailable = menus.filter(
    (item: any) => item.availability === "TERSEDIA",
  ).length;
    ...
    () =>
      [...menus].sort((a: any, b: any) => {
        if (a.availability !== b.availability)
          return a.availability === "TERSEDIA" ? -1 : 1;
  ...
  return (
    <div className="h-screen overflow-hidden bg-[radial-gradient(circle_at_12%_8%,#1a2230_0%,#0b0f16_42%,#090c12_100%)] text-[#eef2f7]">
      <div
        className="mx-auto h-full w-full max-w-[1920px]"
        style={{ padding: tvSafePadding }}
      >
            ...
            <div className="flex items-center gap-4 lg:gap-8">
              <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:block">
                  <h1 className="text-xl lg:text-2xl font-black tracking-tight text-[#ffd0ba] leading-tight">
                    Sego Pedes Basman
                  </h1>
                  <p className="text-xs lg:text-sm font-semibold text-[#9da6b8]">
                    Info Antrian & Status Menu
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 lg:gap-6">
              <div className="flex items-center gap-4 rounded-xl border border-[#313b4e] bg-[#141b28] px-4 py-2">
                <div className="text-right flex flex-col justify-center">
                  <p className="text-2xl lg:text-3xl font-black leading-none text-[#ffc3a6]">
              ...
              <div className="inline-flex rounded-xl border border-[#313b4e] bg-[#131a26] p-1.5 shrink-0">
                <button
                  ...
                  className={`rounded-lg px-4 lg:px-6 py-2 lg:py-2.5 text-sm lg:text-base font-bold transition ${
                    mode === "antrian"
                      ? "bg-[#ff6f3f] text-white shadow-md"
                      : "text-[#94a0b6] hover:text-[#eef2f7]"
                  }`}
                >
                  Antrian
                </button>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mb-3 rounded-xl border border-[#2b3342] bg-[#141b28] px-4 py-2.5 shrink-0">
        <p className="text-sm lg:text-base font-semibold text-[#8b95a8]">
          Silakan perhatikan layar. Nomor antrian akan dipanggil ketika pesanan
          siap diproses.
        </p>
      </div>
        ...
        {paddedOrders.map((order: any, idx: number) => {
          ...
          if (!order) {
            return (
              <div
                key={`empty-${idx}`}
                className="rounded-2xl border border-dashed border-[#2b3342] bg-[#0f1520] flex flex-col items-center justify-center p-3 opacity-50 min-h-0"
              >
                <p className="text-[10px] lg:text-xs font-semibold uppercase tracking-wide text-[#8b95a8] mb-1.5">
                  Belum Tersedia
                </p>
                <div className="h-1 w-8 rounded-full bg-[#313b4e] mb-2" />
                <p className="text-5xl lg:text-[4.5rem] leading-none font-black text-[#313b4e]">
                  -
                </p>
                <p className="mt-2 text-lg font-bold text-[#313b4e] leading-tight">
                  Kosong
                </p>
          ...
          return (
            <div
              key={order.id}
              className={`rounded-2xl border flex flex-col items-center justify-center p-3 min-h-0 ${isFirst ? "border-[#ff6f3f] bg-[#ff6f3f]/10 shadow-[0_0_20px_rgba(255,111,63,0.2)]" : "border-[#2b3342] bg-[#141b28]"}`}
            >
              <p
                className={`text-[10px] lg:text-xs font-semibold uppercase tracking-wide mb-1.5 text-center ${isFirst ? "text-[#ff935f]" : "text-[#8b95a8]"}`}
              >
                {isFirst ? "Sedang Dipanggil" : "Antrian Berikutnya"}
              </p>
              <div
                className={`h-1 w-8 rounded-full mb-2 ${isFirst ? "bg-[#ff6f3f]" : "bg-[#313b4e]"}`}
              />
              <p
                className={`text-5xl lg:text-[4.5rem] leading-none font-black ${isFirst ? "text-[#eef2f7]" : "text-[#d3dae7]"}`}
              >
                {formatTicketCode(order.nomorAntrian).slice(1)}
              </p>
              <p
                className={`mt-2 truncate max-w-full px-2 text-lg lg:text-xl font-bold leading-tight ${isFirst ? "text-[#ffd0ba]" : "text-[#9da6b8]"}`}
              >
                {order.namaPelanggan}
              </p>
              <div
                className={`mt-2.5 h-1 w-16 rounded-full ${isFirst ? "bg-[#ff6f3f]" : "bg-[#313b4e]"}`}
              />
            </div>
          );
        })}
      ...
      <div className="mt-3 grid grid-cols-3 gap-3 shrink-0">
        <div className="rounded-xl border border-[#2b3342] bg-[#141b28] px-4 lg:px-5 py-3 flex items-center justify-between">
          <p className="text-xs lg:text-sm font-semibold text-[#8b95a8]">
            Total Antrian Aktif
          </p>
          <p className="text-xl font-bold text-[#eef2f7]">
            {statsTotal}{" "}
            <span className="text-sm font-medium text-[#8b95a8]">Antrian</span>
          </p>
        </div>
        <div className="rounded-xl border border-[#2b3342] bg-[#141b28] px-4 lg:px-5 py-3 flex items-center justify-between">
          <p className="text-xs lg:text-sm font-semibold text-[#8b95a8]">
          ...
          <p className="text-xl font-bold text-[#eef2f7]">
            ~{avgMinutes}{" "}
            <span className="text-sm font-medium text-[#8b95a8]">Menit</span>
          </p>
        </div>
        <div className="rounded-xl border border-[#2b3342] bg-[#141b28] px-4 lg:px-5 py-3 flex items-center justify-between">
          <p className="text-xs lg:text-sm font-semibold text-[#8b95a8]">
    ...
    <div className="flex h-full flex-col">
      <div className="mb-2 lg:mb-3 rounded-xl border border-[#2b3342] bg-[#141b28] px-3 py-2 flex justify-between items-center shrink-0">
        <p className="text-xs lg:text-sm font-semibold text-[#8b95a8] leading-tight">
          Ketersediaan menu terhubung dengan sistem Dapur Sego Pedes Basman.{" "}
          <span className="text-[#ff935f]">Hanya menampilkan menu Habis.</span>
        </p>
        <div className="flex gap-3">
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-full bg-emerald-500"></div>
            <span className="text-[#d3dae7] text-xs font-semibold">
              {totalAvailable} Tersedia
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-full bg-red-500"></div>
            <span className="text-[#ff935f] text-xs font-semibold">
              {totalOut} Habis
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-hidden">
        {Object.keys(groupedMenus).length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <h2 className="text-xl lg:text-2xl font-bold text-[#eef2f7]">
              Semua Menu Tersedia
            </h2>
            <p className="text-[#8b95a8] mt-2">
              Saat ini tidak ada menu yang habis/kosong.
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-2 lg:gap-2.5 w-full h-full pr-1">
            {Object.entries(groupedMenus).map(
              ([category, items]: [string, any]) => (
                <div
                  key={category}
                  className="shrink-0 flex flex-col border border-[#2b3342] rounded-lg p-2 lg:p-2.5 bg-[#141b28]/40"
                >
                  <div className="flex flex-col mb-2">
                    <h2 className="text-xs lg:text-sm font-bold uppercase tracking-wider text-[#ff935f] mb-1">
                      {category}
                    </h2>
                  ...
                  <div className="flex flex-row flex-wrap gap-2">
                    {items.map((menu: any) => (
                      <div
                        key={menu.id}
                        className="relative overflow-hidden rounded bg-[#1a2230] px-2.5 py-1 lg:px-3 lg:py-1 flex items-center gap-1.5 border border-[#313b4e] shadow-sm w-fit shrink-0"
                      >
                        <div className="h-1.5 w-1.5 lg:h-2 lg:w-2 rounded-full bg-red-500 shrink-0"></div>
                        <h3 className="text-xs lg:text-sm font-semibold text-[#c3c8d3] truncate">
                          {menu.nama}
                        </h3>
                      </div>
                    ))}
